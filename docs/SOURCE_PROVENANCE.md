...
Provenance lookup follows stored references through citation → section → source version → source. It does not search the web or ingest text. Source-version and citation reads expose derived verification status, reviewer, review time, latest decision and re-review date, never client-editable verification booleans. The owner-approved verification type is EDITORIAL (ADR 0012).
